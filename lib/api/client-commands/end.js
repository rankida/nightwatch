  ...
  command(callback) {
    ...
    if (this.api.sessionId) {
      if (this.testFailuresExist() && this.shouldTakeScreenshot()) {
        try {
          var fileNamePath = Screenshots.getScreenshotFileName(client.api.currentTest, false, client.options.screenshots.path);
          Logger.info('Failures in "' + this.api.currentTest.name + '". Taking screenshot...');
        }
        catch (err) {
          Logger.warn('Error generating screenshot filename...', err);
